...
f(i,n-1){
    for(int j=i+1;j<n;j++){
        if(mpp.find(sum-v[i]-v[j])!=mpp.end()){
              if(mpp[sum-v[i]-v[j]].first!=i&&mpp[sum-v[i]-v[j]].first!=j&&mpp[sum-v[i]-v[j]].second!=j&&mpp[sum-v[i]-v[j]].second!=i){
                   cout<<i+1<<' '<<j+1<<' '<<mpp[sum-v[i]-v[j]].first+1<<' '<<mpp[sum-v[i]-v[j]].second+1;
                   return;
               }
            }
        }}
