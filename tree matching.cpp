...
// ==================== SOLVE FUNCTION ====================
vector<vector<int>>adj;                //transition is a bit tricky else simple
vector<int>vis;
vector<vector<int>>dp;

void dfs(int a,int parent  ){
    vis[a]=1;
     dp[a][1] = INT_MIN;   //missed this,,imp
    for(auto x: adj[a]){
        
        if(vis[x]==0){
            dfs(x,a);
            dp[a][0]+=max(dp[x][0],dp[x][1]);
            dp[a][1]=max(dp[a][1],min(0,dp[x][0]-dp[x][1]));
