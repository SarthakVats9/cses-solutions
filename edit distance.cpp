...
void solve() {
    string n , m ;
    cin >> n >> m;
    vector<vector<ll>>dp(n.size()+1,vector<ll>(m.size()+1,1e9));
    //the state of dp[][] tells the min no. of operations till ith in first and jth in 2nd string
    //there gotta be 3 transitions ,,,,,replace delete and add
    dp[0][0]=0;
    ...
    for(ll i = 1 ; i<=n.size();i++){
        for(ll j = 1 ; j<= m.size();j++){
            dp[i][j]=min(dp[i][j],1+dp[i-1][j]);//delete
            dp[i][j]=min(dp[i][j],1+dp[i][j-1]);//add
            dp[i][j]=min(dp[i][j],(n[i-1]!=m[j-1])+dp[i-1][j-1]);//replace
            //1 based and 0based indexing diff
        }
    }
